function generateDomain3DistractorQuestions() {
  ...
  const templates = [
    ...
    { task: "3.4", q: (c, k) => `${c} needs virtually unlimited key-value scale with single-digit millisecond performance for high-traffic product catalog metadata. Which service is the best match for ${k}?`, opts: ["Amazon DynamoDB", "Amazon RDS for PostgreSQL", "Amazon EFS", "Amazon Athena"], ans: "Amazon DynamoDB", next: "Amazon RDS for PostgreSQL", win: "DynamoDB provides managed NoSQL key-value performance and scale for this access pattern.", lose: "RDS can serve catalogs but scaling and operational patterns are less aligned for this workload profile." },
    ...
    { task: "3.6", q: (c, k) => `${c} needs object storage for static assets with 11 9s durability and web-scale access patterns. Which service best fits ${k}?`, opts: ["Amazon S3", "Amazon EBS", "Amazon EFS", "Amazon FSx for Windows File Server"], ans: "Amazon S3", next: "Amazon EFS", win: "S3 is designed for durable, scalable object storage and static content use cases.", lose: "EFS is shared file storage, not object storage optimized for internet-scale static asset distribution." },
