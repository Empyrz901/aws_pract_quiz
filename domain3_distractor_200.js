function generateDomain3DistractorQuestions() {
  ...
  const templates = [
    { task: "3.1", q: (c, k) => `${c} wants repeatable infrastructure provisioning across environments. What AWS service best matches that need for ${k}?`, opts: ["AWS CloudFormation", "AWS CloudFormation StackSets", "AWS Service Catalog", "AWS Systems Manager Automation"], ans: "AWS CloudFormation", next: "AWS CloudFormation StackSets", win: "CloudFormation provides infrastructure as code with repeatability and change tracking.", lose: "StackSets extend CloudFormation, but CloudFormation is the core service that directly provisions the stacks." },
    { task: "3.1", q: (c, k) => `${c} needs a managed way to deploy web apps without managing infrastructure directly. Which service best supports ${k}?`, opts: ["AWS Elastic Beanstalk", "AWS Systems Manager Automation", "Amazon ECS on Fargate", "AWS CloudFormation"], ans: "AWS Elastic Beanstalk", next: "Amazon ECS on Fargate", win: "Elastic Beanstalk provides managed application deployment workflows and environment orchestration.", lose: "Fargate reduces ops for containers, but Beanstalk is the closest fit for managed web app deployment." },
    ...
    { task: "3.3", q: (c, k) => `${c} runs stateless APIs with highly variable demand and wants to avoid server management. Product engineering requests ${k}. Which compute option is most suitable?`, opts: ["AWS Lambda", "AWS Elastic Beanstalk", "Amazon ECS on Fargate", "Amazon EC2 Auto Scaling"], ans: "AWS Lambda", next: "Amazon ECS on Fargate", win: "Lambda eliminates server management and scales automatically for event-driven API patterns.", lose: "Fargate reduces operational work, but Lambda is the most serverless fit for this API pattern." },
    { task: "3.3", q: (c, k) => `${c} needs containerized workloads without provisioning or patching cluster worker nodes. The platform team requires ${k}. Which service is best?`, opts: ["AWS Fargate", "Amazon ECS on EC2 instances", "Amazon EKS on managed node groups", "AWS Elastic Beanstalk"], ans: "AWS Fargate", next: "Amazon EKS on managed node groups", win: "Fargate provides serverless containers with no worker node management.", lose: "Managed node groups reduce ops, but Fargate removes node management entirely." },
    { task: "3.4", q: (c, k) => `${c} needs a managed relational database that supports MySQL, PostgreSQL, Oracle, and SQL Server without managing database hardware. The team wants ${k}. Which service should they choose?`, opts: ["Amazon RDS", "Amazon Aurora", "Amazon EC2 self-managed MySQL", "Amazon Redshift"], ans: "Amazon RDS", next: "Amazon Aurora", win: "RDS supports multiple relational engines and handles backups, patching, and failover for you.", lose: "Aurora is also managed, but this question is asking for the broader relational service family." },
    { task: "3.4", q: (c, k) => `${c} stores session state with millisecond latency requirements and needs an in-memory managed datastore. The workload demands ${k}. Which service is most appropriate?`, opts: ["Amazon ElastiCache", "Amazon DynamoDB", "Amazon DynamoDB Accelerator (DAX)", "Amazon Aurora"], ans: "Amazon ElastiCache", next: "Amazon DynamoDB", win: "ElastiCache is purpose-built for low-latency in-memory caching and session storage.", lose: "DynamoDB is scalable, but ElastiCache is the canonical fit for session state." },
    { task: "3.4", q: (c, k) => `${c} needs virtually unlimited key-value scale with single-digit millisecond performance for high-traffic product catalog metadata. Which service is the best match for ${k}?`, opts: ["Amazon DynamoDB", "Amazon Aurora", "Amazon ElastiCache", "Amazon RDS"], ans: "Amazon DynamoDB", next: "Amazon Aurora", win: "DynamoDB provides managed NoSQL key-value performance and scale for this access pattern.", lose: "Aurora is relational, but DynamoDB is the best fit for this key-value scale." },
    ...
    { task: "3.5", q: (c, k) => `${c} needs to route HTTP and HTTPS traffic to healthy targets based on URL path rules. The solution must provide ${k}. Which service should be used?`, opts: ["Application Load Balancer", "Network Load Balancer", "Amazon Route 53", "AWS Global Accelerator"], ans: "Application Load Balancer", next: "Network Load Balancer", win: "Application Load Balancer supports path-based routing for HTTP and HTTPS traffic.", lose: "Network Load Balancer is a Layer 4 load balancer and does not inspect URL paths." },
    ...
    { task: "3.1", q: (c, k) => `${c} wants a managed service for deploying and managing web application environments without dealing with servers directly. Which service should they pick for ${k}?`, opts: ["AWS Elastic Beanstalk", "AWS CloudFormation", "Amazon CloudWatch", "AWS Systems Manager Automation"], ans: "AWS Elastic Beanstalk", next: "AWS CloudFormation", win: "Elastic Beanstalk provides managed application deployment workflows and environment orchestration.", lose: "CloudFormation defines infrastructure, but Beanstalk is the direct managed app environment service." }
  ];
